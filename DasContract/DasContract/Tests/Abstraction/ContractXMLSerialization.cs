...
using DasContract.Abstraction.XML.Factory;
using NUnit.Framework;

namespace DasContract.Tests.Abstraction
{
    public class ContractXMLSerialization
    {
        ContractDataModel GetExampleDataModel()
        {
            var dataModel = new ContractDataModel();

            dataModel.Id = "data-model-1";
            ...
            var entity1 = new ContractEntity()
            {
                ...
                PrimitiveProperties = new List<PrimitiveContractProperty>()
                {
                    new PrimitiveContractProperty()
                    {
                         ...
                         Name = "Property 1",
                         IsMandatory = true,
                         Type = PrimitiveContractPropertyType.Int
                    },
                    new PrimitiveContractProperty()
                    {
                         Id = "property-2",
                         Name = "Property 2",
                         IsMandatory = false,
                         Type = PrimitiveContractPropertyType.IntCollection
                    },
                },
                ...
                CollectionReferenceProperties = new List<CollectionReferenceContractProperty>() 
                {
                    new CollectionReferenceContractProperty()
                    {
                         ...
                         Name = "Property 2",
                         IsMandatory = false,
                         EntityIds = new List<string>(){ "entity-2" }
                    }
                },
            };
        ...
        {
            Assert.AreEqual("data-model-1", dataModel.Id);
            ...
            Assert.AreEqual(PrimitiveContractPropertyType.Int, entity1.PrimitiveProperties[0].Type);
            ...
            Assert.AreEqual(PrimitiveContractPropertyType.IntCollection, entity1.PrimitiveProperties[1].Type);

            Assert.AreEqual("property-1", entity1.ReferenceProperties[0].Id);
            Assert.AreEqual("Property 1", entity1.ReferenceProperties[0].Name);
            Assert.IsTrue(entity1.ReferenceProperties[0].IsMandatory);
            Assert.AreEqual("entity-2", entity1.ReferenceProperties[0].EntityId);

            Assert.AreEqual("property-2", entity1.CollectionReferenceProperties[0].Id);
            Assert.AreEqual("Property 2", entity1.CollectionReferenceProperties[0].Name);
        ...
        Process GetExampleProcess()
        {
            return ProcessFactory.FromBPMN(GetExampleBPMN()).First();
        }

        void AssertExampleProcess(Process process)
        {
            ...
            Assert.AreEqual(process, process);
        }
        ...
        public void SerializationWithNoDiagram()
        {
            ...
            //Serialize
            var serializedContract = ContractXML.ToXML(contract);

            //Deserialize
            var deserializedContract = ContractXML.FromXML(serializedContract);
        ...
        public void SerializationWithDiagram()
        {
            var contract = new Contract()
            {
                ...
                {
                    Diagram = GetExampleBPMN()
                }
            };
            contract.Processes.Main.UserActivities.First().DueDateExpression = "XXX";
            ...
            Assert.AreEqual("Contract", deserializedContract.Name);
            AssertExampleDataModel(deserializedContract.DataModel);
            Assert.AreEqual(GetExampleBPMN(), deserializedContract.Processes.Diagram);
            Assert.AreEqual("XXX", deserializedContract.Processes.Main.UserActivities.First().DueDateExpression);
            AssertExampleProcess(deserializedContract.Processes.Main);
        }
    }
}
