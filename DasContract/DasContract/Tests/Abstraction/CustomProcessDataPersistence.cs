...
namespace DasContract.Tests.Abstraction
{
    public class CustomProcessDataPersistence
    {
        ...
        public void SerializationWithDiagram()
        {
            ...
            contract.Processes.Main.BusinessActivities.First().BusinessRuleDefinitionXml = "YYY";
            contract.Processes.Main.ScriptActivities.First().Script = "ZZZ";

            contract.Processes.Diagram = GetExampleBPMN();

            //Check
            Assert.AreEqual("XXX", contract.Processes.Main.UserActivities.First().DueDateExpression);
            Assert.AreEqual("YYY", contract.Processes.Main.BusinessActivities.First().BusinessRuleDefinitionXml);
